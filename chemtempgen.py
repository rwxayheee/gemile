...
# for amber (parse and construct mols from OFF lib)
import parmed
...
def embed(mol: Chem.Mol, allowed_smarts: str, 
    ...
    """
    Remove atoms from the molecule based the union of matches by
...
def cap(mol: Chem.Mol, allowed_smarts: str, 
        capping_names: set[str] = set(), capping_smarts_loc: dict[str, set[int]] = {}, 
        protonate: bool = False) -> Chem.Mol:
    """Add hydrogens to atoms with implicit hydrogens based on the union of matches by
...
def get_pretty_smiles(smi: str) -> str: 
    ...
    for content in contents:
        # keep [ ] for charged atoms
        if any(charge_symbol in content for charge_symbol in ("+", "-")):
            continue
        ...
        if is_element(H_stripped) and is_element(H_stripped) not in metal_AtomicNums: 
...
class ChemicalComponent:
    ...
    def __eq__(self, other):
        if isinstance(other, ChemicalComponent):
            return self.smiles_exh == other.smiles_exh
        return False
    
    @classmethod
    def from_lib(cls, source_lib: str, resname: str):
        """Create ChemicalComponent from an Amber OFF library file and a searchable residue name in file."""
        ...
        def guess_bond_order(residue: parmed.modeller.residue.ResidueTemplate, max_attempts = 5) -> parmed.modeller.residue.ResidueTemplate: 
            ...
            num_attempts = 0
            while len(unspec_bonds)>0 and num_attempts < max_attempts: 
                num_attempts += 1
...
def build_linked_CCs(basename: str, embed_allowed_smarts: str = None, 
    ...
    with ChemicalComponent_LoggingControler(): 
        ...
        if not embed_allowed_smarts:
            ...
            if all(item for item in (cap_allowed_smarts, pattern_to_label_mapping_standard, variant_dict)):
